...
function App() {
  ...
  const isProgressBarShown = pathname === config.ticketsUrl || pathname === config.seatsUrl || pathname === config.passengersUrl || pathname === config.paymentUrl || pathname === config.orderUrl;
  ...
  const filterTicketsIsLoading = useSelector((state) => state.filterTickets.isLoading);
  ...
  return (
    <>
      ...
      <main className='main'>
        <Hero />
        {isHomePage ? <Loader isHomePage={isHomePage} /> : (isTicketsPage || isSeatsPage || isOrderPage) && isLoading ? <Loader isHomePage={isHomePage} /> : ''}
