...
function SubscriptionForm() {
    ...
    const sendEmailData = async () => {
        try {
            const response = await fetch(import.meta.env.VITE_APP_URL + import.meta.env.VITE_SUBSCRIBE_REQ + form.email, {
                method: 'POST',
                headers: {
        ...
        <div className='footer__subscription'>
            ...
            <form className='footer__subscription-form' onSubmit={onSubmit}>
                <label className='footer__subscription-label'>
                    <span className='footer__subscription-label-text'>Будьте в курсе событий</span>
                    <input type='text' className='footer__subscription-input' name='email' placeholder='e-mail' value={form.email} onChange={onValueChange} />
                </label>
