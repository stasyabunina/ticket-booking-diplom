...
import { removeOrderSeat, removeSeat } from '../../redux/actions/actionCreators';

function PassengerCollapse(props) {
    const dispatch = useDispatch();
    const { isExpanded, index, route, seatNumber, coachId, isChild, isSuccessShown } = props;
    ...
    const onRemoveSeatClick = () => {
        ...
        dispatch(removeSeat(seatNumber, route, coachId, isChild === true ? 'childTicketsTotal' : 'adultTicketsTotal'));
    }

    return (
        <div className='passenger__collapse'>
            <button className='passenger__collapse-btn' type='button' onClick={() => setCollapse(prev => !prev)}>
                {collapse ? <CollapseExpanded /> : <CollapseNotExpanded />}
                <h3 className='passenger__collapse-title'>Пассажир {index}</h3>
            </button>
            <div className={`passenger__collapse-container${collapse ? ' passenger__collapse-container--expanded' : ''}`}>
                {props.children}
            </div>
            {collapse ? <button className='passenger__remove-btn' type='button' onClick={onRemoveSeatClick}>
