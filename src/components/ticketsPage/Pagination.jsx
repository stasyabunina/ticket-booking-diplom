...
import { updateCurrentPage, updateOffset } from '../../redux/actions/actionCreators';
...
function Pagination() {
    ...
    return (
        <div className='tickets__pagination'>
            <button className='tickets__pagination-btn tickets__pagination-btn_prev' type='button' onClick={() => onPrevClick(totalPages)}>
            ...
            {currentPage === 1 ? '' : <button className='tickets__pagination-btn' type='button' onClick={() => onPrevClick(1)}>
                {currentPage - 1}
            </button>}
            <button className='tickets__pagination-btn tickets__pagination-btn_current' type='button'>
                {currentPage}
            </button>
            {currentPage + 1 > totalPages ? '' : totalPages >= 2 ? <button className='tickets__pagination-btn' type='button' onClick={() => onNextClick(1)}>
                {currentPage + 1}
            </button> : ''}
            {currentPage + 2 > totalPages ? '' : totalPages >= 3 ? <button className='tickets__pagination-btn tickets__pagination-btn_third' type='button' onClick={() => onNextClick(2)}>
                {currentPage + 2}
            </button> : ''}
            {totalPages >= 5 && currentPage <= totalPages - 4 ? <button className='tickets__pagination-btn tickets__pagination-btn_more' type='button'>
            ...
            {totalPages >= 5 && currentPage <= totalPages - 4 ? <button className='tickets__pagination-btn tickets__pagination-btn_last' type='button' onClick={() => onNextClick(totalPages)}>
                {totalPages}
            </button> : ''}
            <button className='tickets__pagination-btn tickets__pagination-btn_next' type='button' onClick={() => onNextClick(1)}>
                <svg width='18' height='29' viewBox='0 0 18 29' fill='none' xmlns='http://www.w3.org/2000/svg'>
