...
function TicketsPage() {
    ...
    const { items, isLoading, error, success } = useSelector((state) => state.filterTickets);
    const { sort, limit, offset, from_city_id, to_city_id, have_air_conditioning, have_express, have_first_class, have_fourth_class, have_second_class, have_third_class, have_wifi } = useSelector((state) => state.searchTickets.form);
    ...
    }, [sort, limit, offset, have_air_conditioning, have_express, have_first_class, have_fourth_class, have_second_class, have_third_class, have_wifi, from_city_id, to_city_id]);
        ...
        <section ref={section} className='dashboard tickets'>
            <div className='container'>
                <div className='dashboard__wrapper tickets__wrapper'>
                    {!isFormValid ? '' : <Aside />}
                    {!isFormValid ? '' : isErrorShown ? <TicketsError error={error} onErrorClick={onErrorClick} /> : items.length === 0 && success && !isLoading ? <span>Ничего не найдено.</span> : items.length === 0 && !success && !isLoading ? '' : isLoading ? <span>Идет загрузка...</span> : <Tickets />}
                </div>
            </div>
        </section>
