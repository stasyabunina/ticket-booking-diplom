      ...
      <div className='container'>
        <div className='dashboard__wrapper seats__wrapper'>
          ...
          <div className='seats__results'>
            ...
            {departure !== 0 && item !== '' ? <div className='seats__train'>
              <TrainRoute item={item} coaches={departure} route='departure' onBackBtn={onBackBtn} />
              {item.arrival ? <TrainRoute item={item} coaches={arrival} route='arrival' onBackBtn={onBackBtn} /> : ''}
            </div> : ''}
            <button className='btn next-btn seats__next-btn' type='button' disabled={seats.length !== 0 && totalAdultPassengers ? false : true} onClick={onNextClick}>Далее</button>
          </div>
        </div>
      </div>
    </section>
