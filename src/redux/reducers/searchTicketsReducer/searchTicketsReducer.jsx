...
export default function searchTicketsReducer(state = initialState, action) {
    let offset, currentPage, limit, sort, name, value, min, minValue, max, maxValue, cityFrom, cityTo, nameDate, valueDate, nameFrom, cityIdFrom, nameTo, cityIdTo, citiesFrom, citiesTo, error;
    ...
    switch (action.type) {
        ...
        case UPDATE_SLIDER_VALUE:
            ...
            return { ...state, form: { ...state.form, [min]: minValue, [max]: maxValue } };
        case SWAP_VALUES:
            return { ...state, form: { ...state.form, date_start: state.form.date_end, date_end: state.form.date_start, from_city_id: state.form.to_city_id, to_city_id: state.form.from_city_id }, cityFrom: state.cityTo, cityTo: state.cityFrom };
        ...
        case SELECT_WHERE_FROM_CITY:
            ...
            return { ...state, form: { ...state.form, from_city_id: cityIdFrom }, cityFrom: nameFrom };
        case SELECT_WHERE_TO_CITY:
            ({ nameTo, cityIdTo } = action.payload);
            return { ...state, form: { ...state.form, to_city_id: cityIdTo }, cityTo: nameTo };
